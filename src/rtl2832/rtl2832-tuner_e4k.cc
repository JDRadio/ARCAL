...
e4k::e4k(demod* p)
	: tuner_skeleton(p)
{
	for (unsigned int i = 0; i < sizeof(_mapGainsE4K)/sizeof(_mapGainsE4K[0]); i += 2)
...
int e4k_compute_pll_params(struct e4k_pll_params *oscp, uint32_t fosc, uint32_t intended_flo)
{
	...
	for (unsigned int i = 0; i < ARRAY_SIZE(vco_r_table_3ph); i++) {
...
int e4k_dc_offset_gen_table(struct e4k_state *e4k)
{
	...
	for (unsigned int i = 0; i < ARRAY_SIZE(dc_gain_comb); i++) {
